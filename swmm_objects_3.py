# AEM Modified 4/30/18 for PYTHON 3
...
#		for key in self.catdict.keys():
			#print(key)
		catdictKeysList = list(self.catdict.keys())  # Python 3 requires putting keys in a list
		#print(catdictKeysList)
		catdictValuesList = list(self.catdict.values())  # Python 3 requires putting values in a list
		for obj in self.catclasses:
			heading = catdictKeysList[catdictValuesList.index(obj)]  # return key associated with dict value
...
#		return lid_area		
	def getLidSubcatchmentNameList(self):
		lidUsageClass = self.moddict[lid_usage]
		subcatAndLidList = lidUsageClass.getNameList() # a list of tuples (subcatchment, LID name)
		lidSubcatNameList = []
		for pair in subcatAndLidList:
			subcatName = pair[0]
			if not (subcatName in lidSubcatNameList):
				lidSubcatNameList.append(subcatName)
		return lidSubcatNameList
